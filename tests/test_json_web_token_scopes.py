...
class FlaskOAuthProviderJWTScopesTestCase(FlaskOAuthProviderBaseTestCase):
    def test_auth_insufficient_token_scopes(self):
        error = ApiAuthTokenScopesInsufficient(meta={"required_scopes": ["unobtainable-scope"], "scopes_in_token": []})
        ...
        self._check_error_response(response=response, json_response=json_response, expected_status=HTTPStatus.FORBIDDEN)
